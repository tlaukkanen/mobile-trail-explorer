...
public class TrailCanvas extends BaseCanvas implements Runnable {
    ...
    private void drawWaypoints(Graphics g) {
        ...
        for (int waypointIndex = 0; waypointIndex < waypointCount; waypointIndex++) {
            ...
            if (point != null) {
                g.drawString(waypoint.getName(), point.X + 2, point.Y - 1, Graphics.BOTTOM
                        | Graphics.LEFT);
    ...
    private void drawZoomScaleBar(Graphics g) {
        ...
        double scale[] = (double[]) zoomScaleBarDefinition.get(Integer
                .toString(horizontalZoomFactor));
        ...
        g.drawLine(MARGIN_LEFT, getHeight() - MARGIN_BOTTOM, MARGIN_LEFT + scaleLength, getHeight()
                - MARGIN_BOTTOM);
        g.drawLine(MARGIN_LEFT, getHeight() - MARGIN_BOTTOM, MARGIN_LEFT, getHeight()
                - MARGIN_BOTTOM - 3);
        g.drawLine(MARGIN_LEFT + scaleLength, getHeight() - MARGIN_BOTTOM, MARGIN_LEFT
                + scaleLength, getHeight() - MARGIN_BOTTOM - 3);

        /* Divide the complete scale bar into smaller parts */
        int scalePartLength = (int) (scaleLength / scaleParts);
        for (int i = 1; i < scaleParts; i++) {
            g.drawLine(MARGIN_LEFT + scalePartLength * i, getHeight() - MARGIN_BOTTOM, MARGIN_LEFT
                    + scalePartLength * i, getHeight() - MARGIN_BOTTOM - 2);
        }

        /* Build text for the right end of the scale bar and get width of this text */
        String text = "", unit = "";
        if (settings.getUnitsAsKilometers()) {
            text = scale[0] >= 1000 ? Integer.toString((int) (scale[0] / 1000)) : Integer
                    .toString((int) scale[0]);
            unit = scale[0] >= 1000 ? "km" : "m";
        } else {
            text = scale[3] >= 1600 ? Integer.toString((int) (scale[3]
                    / UnitConverter.KILOMETERS_IN_A_MILE / 1000)) : Integer
                    .toString((int) (scale[3] / UnitConverter.METERS_IN_A_FOOT));
            unit = scale[3] >= 1600 ? "ml" : "ft";
        }
        int textWidth = g.getFont().stringWidth(text);

        g.drawString("0", MARGIN_LEFT - 1, getHeight() - MARGIN_BOTTOM - 2, Graphics.BOTTOM
                | Graphics.LEFT);
        g.drawString(text + unit, MARGIN_LEFT + scaleLength - textWidth / 2, getHeight()
                - MARGIN_BOTTOM - 2, Graphics.BOTTOM | Graphics.LEFT);
    }
    ...
    private void drawStatusBar(Graphics g) {
        ...
        if (lastPosition != null) {
            ...
            if (settings.getDisplayValue(RecorderSettings.DISPLAY_COORDINATES) == true) {
                ...
                StringUtil.valueOf(latitude, 5), positionAdd, fontHeight, Graphics.TOP
                        | Graphics.LEFT);

                double longitude = lastPosition.longitude;
                g.drawString(
                /* Get degrees in string format (with five decimals) */
                StringUtil.valueOf(longitude, 5), positionAdd, fontHeight * 2, Graphics.TOP
                        | Graphics.LEFT);
            ...
            if (settings.getDisplayValue(RecorderSettings.DISPLAY_SPEED) == true) {
                ...
                if (settings.getUnitsAsKilometers() == false) {
                    speed = (int) UnitConverter.convertSpeed(lastPosition.speed,
                            UnitConverter.KILOMETERS_PER_HOUR, UnitConverter.MILES_PER_HOUR);
                    units = " mph";
                } else {
                    speed = (int) lastPosition.speed;
                    units = " km/h";
                }
                g.drawString("SPD:", 1, fontHeight * displayRow, Graphics.TOP | Graphics.LEFT);
                g.drawString(speed + units, positionAdd, fontHeight * displayRow, Graphics.TOP
                        | Graphics.LEFT);
                displayRow++;
            }
            ...
            if (settings.getDisplayValue(RecorderSettings.DISPLAY_HEADING) == true) {
                ...
                g.drawString("HEA:", 1, fontHeight * displayRow, Graphics.TOP | Graphics.LEFT);
                g.drawString(heading, positionAdd, fontHeight * displayRow, Graphics.TOP
                        | Graphics.LEFT);
                displayRow++;
            }
            ...
            if (settings.getDisplayValue(RecorderSettings.DISPLAY_DISTANCE) == true) {
                ...
                if (settings.getUnitsAsKilometers() == false) {
                    /** Distance in feets */
                    double distanceInMiles = UnitConverter.convertLength(distanceInKilometers,
                            UnitConverter.KILOMETERS, UnitConverter.MILES);
                ...
                g.drawString("DST:", 1, fontHeight * displayRow, Graphics.TOP | Graphics.LEFT);
                g.drawString(distance + units, positionAdd, fontHeight * displayRow, Graphics.TOP
                        | Graphics.LEFT);
                displayRow++;
            }
            ...
            if (settings.getDisplayValue(RecorderSettings.DISPLAY_ALTITUDE) == true) {
                ...
                if (settings.getUnitsAsKilometers() == false) {
                    ...
                    double altitudeInFeets = UnitConverter.convertLength(altitudeInMeters,
                            UnitConverter.METERS, UnitConverter.FEETS);
                ...
                g.drawString("ALT:", 1, fontHeight * displayRow, Graphics.TOP | Graphics.LEFT);
                g.drawString(altitude + units, positionAdd, fontHeight * displayRow, Graphics.TOP
                        | Graphics.LEFT);
                displayRow++;
            }
            ...
            g.drawString("Position data is unavailable. " + counter, 1, fontHeight, Graphics.TOP
                    | Graphics.LEFT);
        }
        ...
        if (controller.getError() != null) {
            g.drawString("" + controller.getError(), 1, height - (fontHeight * 2 + 2), Graphics.TOP
                    | Graphics.LEFT);
        }
        ...
        g.drawString(posCount, getWidth() - 2, height - (fontHeight + 2), Graphics.TOP
                | Graphics.RIGHT);
